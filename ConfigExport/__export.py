...
def codegenerator(schema, outfolder, namespace, suffix):
  if os.path.exists(schema):
    cmd = 'tools\CSharpGeneratorForProton\CSharpGeneratorForProton.exe ' + '-n ' + namespace + ' -f ' + outfolder + ' -p ' + schema + ' -e true -d config_server/protos -b bytes'
    if suffix:
      cmd += ' -t ' + suffix 
    code = os.system(cmd)
    os.remove(schema)      
...
def exportserver():
  ...
  codegenerator('schemaserver.json', 'config_server/ConfigGenerator/Template', 'Ball.Config', 'Template') 
    
def exportclient():
  export(EXPORT_FILES + EXPORT_CLIENT_ONLY, 'json', 'client', 'config_client', 'Template', None)
